...
export default function GirisCikisKayitlariPage() {
  ...
  const copyToClipboard = async () => {
    ...
    filteredRecords.forEach((r, index) => {
      ...
      const tarih = r.tarih instanceof Timestamp ? r.tarih.toDate() : (r.tarih instanceof Date ? r.tarih : new Date());
  ...
  return (
    <div className="min-h-screen bg-white">
      <header className="bg-white border-b px-4 md:px-6 py-4 sticky top-0 z-30">
        <h1 className="text-xl font-bold text-[#2F2F2F]">Giriş ve Çıkış Kayıtları</h1>
        <p className="text-sm text-[#8A8A8A] mt-1">Bu sayfada, belirlediğiniz parametrelere göre "Giriş ve Çıkış Kayıtları" raporunu görüntüleyebilirsiniz.</p>
      </header>
      ...
      <main className="p-4 md:p-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border p-4 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            <div>
              <label className="block text-xs text-[#8A8A8A] mb-1">Kullanıcı seçiniz</label>
              <select
            ...
            <div>
              <label className="block text-xs text-[#8A8A8A] mb-1">İşlem türü</label>
              <select
            ...
            <table className="w-full">
              <thead className="bg-[#F7F7F7] border-b">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-[#8A8A8A] uppercase">Saat</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[#8A8A8A] uppercase">İşlem Türü</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[#8A8A8A] uppercase">Konum Dışı</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[#8A8A8A] uppercase">Mazeret Notu</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#E5E5E5]">
                {filteredRecords.length === 0 ? (
                  <tr>
                    <td colSpan={10} className="px-4 py-12 text-center text-[#8A8A8A]">
                  ...
                  filteredRecords.map((record, index) => {
                    ...
                    const tarih = record.tarih instanceof Timestamp ? record.tarih.toDate() : (record.tarih instanceof Date ? record.tarih : new Date());
                    ...
                    return (
                      <tr key={record.id} className="hover:bg-[#F7F7F7]">
                        ...
                        <td className="px-4 py-3 text-sm text-[#2F2F2F]">
                          {record.konumDisi && <span className="text-[#E6B566]">⚠️</span>}
                        </td>
